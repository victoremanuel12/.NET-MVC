...
namespace MVC2022.Areas.Admin.Controllers
{
    ...
    public class AdminPedidosController : Controller
    {
        ...
        //}
        //Aqui em cima retornava todos os dados de uma vez só

        //Aqui em baixo está retornando uma view com paginação
        public async Task<IActionResult> Index(string filter, int pageIndex = 1, string sort = "Nome")
        {
            //Apenas montou a consulta, sem receber os dados ainda
            //AsNoTracking as entidades nao sao rastreadas pelo contexto, assim o EF CORE nao realizanenhum processamento adicional nas entidades
            //apenas está listando as informações
            // o AsQueryable converte essa operação para um tipo IQueryable, com esse metodos os dados so serão obitidos quando a fonte for avaliada
            var resultado = _context.Pedidos.AsNoTracking().AsQueryable();
            if (!string.IsNullOrEmpty(filter))
            {
                resultado = resultado.Where(p => p.Nome.Contains(filter));
            }
            // Criação do objeto de paginação
